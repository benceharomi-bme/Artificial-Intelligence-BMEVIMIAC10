...
def read_input():
    ...
    for a in range(number_of_nodes):
        ...
        if(number_of_parents == 0):
            ...
            nodes.append(
                Node(is_discrete=True, distribution=dictionary, parents=parents, k=k, id=a))
        else:
            array = []
            ...
            node = Node(is_discrete=False, probability_table=array,
                        parents=parents, k=k, id=a)
            nodes.append(node)
...
def calculate(node: Node):
    ...
    for parent in node.parents:
        if(parent.value != None):
            table = filterTable(
                table, node.parents.index(parent), parent.value)
...
def calculateChildren(nodes, node, calculated):
    for child in node.children:
        if(nodes[child].value != None):
            table = filterTable(
                nodes[child].probability_table, 0, nodes[nodes.index(node)].value)
...
def main():
    ...
    for i in range(nodes[wanted_node_index].k):
        ...
        calculated = calculateChildren(
            nodes, nodes[wanted_node_index], calculated)
